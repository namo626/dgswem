...
contains
   ...
   function evalf(i, Val) result(res)
      ...
      do IP = 1, Comp(i)%ByteCodeSize
         ...
         case (cDiv); if (Comp(i)%Stack(SP) == 0._sz) then; EvalErrType = 1; res = zero; return; end if
         ...
         case (cLog10); if (Comp(i)%Stack(SP) <= 0._sz) then; EvalErrType = 3; res = zero; return; end if
            Comp(i)%Stack(SP) = log10(Comp(i)%Stack(SP))
         case (cLog); if (Comp(i)%Stack(SP) <= 0._sz) then; EvalErrType = 3; res = zero; return; end if
            Comp(i)%Stack(SP) = log(Comp(i)%Stack(SP))
         case (cSqrt); if (Comp(i)%Stack(SP) < 0._sz) then; EvalErrType = 3; res = zero; return; end if
